...
class ExpandableLinearLayout @JvmOverloads constructor(
    ...
    defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr), ExpandableLayout {
     ...
     *
     * Getter: Gets the width from left of layout if orientation is horizontal.
     * Gets the height from top of layout if orientation is vertical.
     *
     * Setter: Sets the close position directly.
     *
    ...
    fun move(position: Int) {
        move(position, duration.toLong(), interpolator)
    }
     ...
     * @see moveChild
     */
    fun moveChild(index: Int) {
        moveChild(index, duration.toLong(), interpolator)
    }
     ...
     * Expand the layout if `to` is bigger than `from`.
     * Collapse the layout if `from` is bigger than `to`.
     *
     * @param from
     * @param to
     * @param duration
